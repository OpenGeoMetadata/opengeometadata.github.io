...
| Entry Guidelines      | This may be a bounding box or more complex geometry.  For a bounding box, it should use the ENVELOPE(w,e,n,s) format.  For a more complex geometry, it should use the WKT ("well-known text") POLYGON or MULTIPOLYGON format.  Note that WKT polygon vertices should be defined in counter-clockwise order. |
